...
update_status ModulePhysics::PostUpdate()
{
	...
	for(b2Body* b = world->GetBodyList(); b; b = b->GetNext())
	{
		for(b2Fixture* f = b->GetFixtureList(); f; f = f->GetNext())
		{
			...
			if (App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_DOWN) {
				...
				if (f->GetShape()->TestPoint(b->GetTransform(), { PIXEL_TO_METERS(App->input->GetMouseX()), PIXEL_TO_METERS(App->input->GetMouseY()) }) == true) {
					b2MouseJointDef def;
					def.bodyA = ground;
					def.bodyB = b;
	...
	if (App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_REPEAT && mouse_joint != nullptr) {
		...
		b2Vec2 anchor1 = mouse_joint->GetAnchorA();
		b2Vec2 anchor2 = mouse_joint->GetAnchorB();

		App->renderer->DrawLine(METERS_TO_PIXELS(anchor1.x), METERS_TO_PIXELS(anchor1.y), METERS_TO_PIXELS(anchor2.x), METERS_TO_PIXELS(anchor2.y), 255, 255, 255);
	}
